...
//         playing={isPlaying || true} // autoplay in background
//         loop
//         muted={!isPlaying} // muted unless user clicks
//         width="100%"
//         height="auto"
...
//       {/* Center play button with Polish text */}
//       {!isPlaying && (
//         <div
...
const Hero = () => {
  ...
  const [isPlaying, setIsPlaying] = useState(false);
  const [muted, setMuted] = useState(true); // <-- separate muted state
  const [showPauseIcon, setShowPauseIcon] = useState(false);
  const playerRef = useRef(null);
  ...
  const handlePlayVideo = () => {
    ...
    setIsPlaying(true);
    setMuted(false); // <-- unmute when user clicks
  };
  ...
  return (
    ...
    >
      <ReactPlayer
        ref={playerRef}
        url="https://marketersi.cdn.prismic.io/marketersi/Z3zMopbqstJ99Gc4_MARKETERSI-16-9-2-.mp4"
        playing={true} // autoplay in background
        loop
      ...
      {/* Image overlay (click to play with sound) */}
      {!isPlaying && (
           <div
          onClick={handlePlayVideo}
          style={{
      ...
      {isPlaying && showPauseIcon && (
        <div
          onClick={() => {
            setIsPlaying(false);
            setMuted(true); // mute again when paused
          }}
          style={{
            position: "absolute",
            top: "50%",
