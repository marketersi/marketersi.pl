...
const FormOne = ({ setCurrentComponent, form }) => {
  ...
  const handleSelectChange2 = (selectedOption) => {
    ...
    if (
      selectedOption &&
      optionsTwo && // Check if optionsTwo exists before accessing its elements
      optionsTwo.length > 0 && // Ensure optionsTwo has elements
      selectedOption.value === optionsTwo[0].value
    ) {
      setIsModalOpen(false);
    }
  ...
  const handleButtonClick = () => {
    ...
    if (!selectedOption || !selectedOption2) {
      ...
      dispatch(savePriceListFormData(payload));
      setCurrentComponent(2); // Pass 2 to the parent's navigation function
    }
  };
